<template>
    <div class="page">
        <div v-if="page">
            ...
            <input type="text" v-model="page.title" class="title" 
                name="title" placeholder="Enter a Title" />
            <label for="content">Content</label>
            <textarea class="content" name="content" v-model="page.content" 
                placeholder="Enter some Page Content" > </textarea>
            <button @click="deletePage()">Delete Page</button>
            <button @click="savePage()">Save Page</button>
        </div>
        <div v-else>
            <h1>&1arr; To start, Create a New Page!</h1>
        </div>
    </div>
</template>
